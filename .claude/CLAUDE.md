...
| `agent` | `agent.rs`, `agent_model.rs`, `ai_model.rs`, `budget.rs` | State machine (Observe->Evaluate->Think->Act->Stop); multi-layer gating; pluggable LLM backends with `DeterministicPolicy` and `HybridPolicy` |
...
- **Pluggable inference**: `ModelBackend` trait with `OllamaBackend` (qwen2.5:1.5b at localhost:11434), `MockBackend` (deterministic, for tests), plus `DeterministicPolicy` (rule-based fallback) and `HybridPolicy` (combines deterministic + model).
...
- 22 integration tests covering classification, diffing, signals, agent behavior, browser command serialization, and error handling
...
`FillInput { form_id, input_label, value, identity }`, `SubmitForm { form_id, action_label, identity }`, `FormSubmitted { form_id }` (observed, not executed), `ClickAction { label, identity }`, `Wait { reason }`
...
src/
  ...
  agent/
    ...
    ai_model.rs       -- OllamaBackend, MockBackend, DeterministicPolicy, HybridPolicy, prompt construction
    budget.rs         -- Budget check (think/retry/loop)
  browser/
    mod.rs            -- Module exports
    playwright.rs     -- DOM extraction, SelectorHint, BrowserCommand, BrowserResult, execute_browser_action()
  canonical/
    mod.rs            -- Module exports
